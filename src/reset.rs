...
pub fn reset(label: Option<String>, recovory: bool) {
    ...
    println!("You really reset the progress to {}?", label_exists.name);
    ...
    if matches!(input.trim(), "y" | "yes") {
        reset_real(label_exists, recovory);
    }    
}


fn reset_real(label: Label, recovory: bool) {
    let mg_path = mg();
    let current = env::current_dir().unwrap();

    // Check if "recovery" label exists and restore it first
    ...
    println!("\x1b[32mINFO:\x1b[0m Current progress saved to 'recovery' label.");
    ...
    println!("\x1b[33mYou can use 'mg reset' to recover files\x1b[0m");
}

fn rescure() {
    let recovery_path = mg().join("saves").join("recovery.tar.xst");    
    let current = env::current_dir().unwrap();
